...
<div class="orders">
<div class="feedback">
<h2><?PHP echo $_SESSION['userName'];?>'s <?PHP echo $lang['ORD_COMP']?></h2>
<?PHP
$query = "SELECT COUNT(*) as num FROM jobs_bought WHERE `username` = '".mysql_real_escape_string($username)."' AND  `accepted` = 'yes' AND  `feedback_left` = 'yes'";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];
	/* Setup vars for query. */
	$targetpage = "orders_completed"; 	//your file name  (the name of this file)
...
$sql = "select * from jobs_bought WHERE `username` = '".mysql_real_escape_string($username)."' AND  `accepted` = 'yes' AND  `feedback_left` = 'yes' order by id desc LIMIT $start, $limit ";
...
<img src=\"images/information.png\" class=\"dialog-ico\" alt=\"\"/>".$lang['NO_COMP_ORD']."</div>
</div><br />";
}
?>
</div>
...
</div></div><META HTTP-EQUIV = 'Refresh' Content = '0; URL =orders_completed.php'>";
}
}
?>
</div>
