...
include("header.php");
 $result = mysql_query("SELECT * FROM members WHERE `username` = '".mysql_real_escape_string($username)."'");
...
<div class="orders">
<div class="feedback">
<h2><?PHP echo $_SESSION['userName'];?>'s <?PHP echo $lang['ORD_PENDING']?></h2>
<?PHP
$query = "SELECT COUNT(*) as num FROM jobs_bought WHERE `username` = '".mysql_real_escape_string($username)."' AND  `accepted` = 'no'";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];
	/* Setup vars for query. */
	$targetpage = "orders_pending"; 	//your file name  (the name of this file)
...
$sql = "select * from jobs_bought WHERE `username` = '".mysql_real_escape_string($username)."' AND  `accepted` = 'no' order by id desc LIMIT $start, $limit";
...
}else{ echo "".$del_id.";<div class=\"dialog-box-success\">
<div class=\"dialog-left\">
...
</div></div><META HTTP-EQUIV = 'Refresh' Content = '0; URL =orders_pending.php'>";
}
}
?>
</div>
