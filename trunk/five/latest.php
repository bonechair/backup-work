...
include("header.php");
include("filter.php");
echo "<h3>".$lang['LATEST_JOBS']."</h3>";
$job_cost = mysql_real_escape_string($_POST['job_cost']);
$sql = "SELECT * FROM jobs,sitesettings WHERE `approved` = 'Yes' ORDER BY id DESC ";
if($job_cost){
$sql = "SELECT * FROM jobs,sitesettings WHERE `approved` = 'Yes' AND `job_cost` = '$job_cost' ORDER BY id desc ";
}
