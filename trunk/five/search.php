...
$sql = "SELECT * FROM jobs,sitesettings WHERE `approved` = 'Yes' AND category LIKE '%$searchterm%' AND `approved` = 'Yes' OR job_cost LIKE '%$searchterm%' AND `approved` = 'Yes' OR username LIKE '%$searchterm%' AND `approved` = 'Yes' OR job_description LIKE '%$searchterm%' AND `approved` = 'Yes' OR willdo LIKE '%$searchterm%' AND `approved` = 'Yes' ";
$result2 = mysql_query($sql)or die(mysql_error());
$tcount = mysql_num_rows($result2);
$tpages = ($tcount) ? ceil($tcount/$rpp) : 1;
$count = 0;
$i = ($page-1)*$rpp;
if($tcount < 1){echo "<h3>".$lang["NO_SEARCH_RESULTS"]." ".$searchterm."</h3>";}else{
echo "<h3>".$lang['SEARCH_RESULTS_FOR']." <b>".$searchterm."</b></h3>";}
