...
<div class="orders">
<div class="feedback">
<h2><? echo $_SESSION['userName'];?>'s <?php echo $lang['SALES_WAIT_FEED']?></h2>
<?PHP
$query = "SELECT COUNT(*) as num FROM jobs_sold WHERE `username` = '".mysql_real_escape_string($username)."' AND  `feedback_left` = 'no'";
...
$sql = "select * from jobs_sold WHERE `username` = '".mysql_real_escape_string($username)."' AND  `feedback_left` = 'no' order by id desc LIMIT $start, $limit";
...
</div></div><META HTTP-EQUIV = 'Refresh' Content = '0; URL =sales_ar.php'>";
}
}
?>
</div>
