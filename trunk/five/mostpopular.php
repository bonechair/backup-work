...
{
$title = "".$row["domain"]." - Most popular jobs";
...
include("header.php");
include("filter.php");
echo "<h3>".$lang['MOST_POPULAR']."</h3>";
$job_cost = mysql_real_escape_string($_POST['job_cost']);
$sql = "SELECT * FROM jobs,sitesettings WHERE `approved` = 'Yes' ORDER BY times_bought DESC ";
if($job_cost){
$sql = "SELECT * FROM jobs,sitesettings WHERE `approved` = 'Yes' AND `job_cost` = '$job_cost' ORDER BY times_bought desc ";
}
