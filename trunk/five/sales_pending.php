...
{
$title = "".$row["domain"]." - ".$_SESSION['userName']."s pending orders";
...
<div class="orders">
<div class="feedback">
<h2><? echo $_SESSION['userName'];?>'s <?PHP echo $lang['SALES_PENDING']?></h2>
<?PHP
$query = "SELECT COUNT(*) as num FROM jobs_sold WHERE `username` = '".mysql_real_escape_string($username)."' AND  `accepted` = 'no'";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];
	/* Setup vars for query. */
	$targetpage = "sales_pending"; 	//your file name  (the name of this file)
...
$sql = "select * from jobs_sold WHERE `username` = '".mysql_real_escape_string($username)."' AND  `accepted` = 'no' order by id desc LIMIT $start, $limit";
...
<img src=\"images/information.png\" class=\"dialog-ico\" alt=\"\"/>".$lang['NO_ORD_WAIT_ACC']."</div>
</div><br />";
}
?>
</div>
...
</div></div><META HTTP-EQUIV = 'Refresh' Content = '0; URL =sales_pending.php'>";
}
}
?>
</div>
