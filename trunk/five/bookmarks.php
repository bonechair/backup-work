...
{
$title = "".$row["domain"]." - ".$_SESSION['userName']."'s Bookmarked Likes!";
...
include("filter.php");
?>
<h2><?PHP echo $lang['BOOKMARKED']?></h2>
<?PHP
$job_cost = mysql_real_escape_string($_POST['job_cost']);
$sql1 = "SELECT * FROM likes WHERE `username` = '$_SESSION[userName]'  ";
$my_likes = mysql_query($sql1)or die(mysql_error());
while($myrow = mysql_fetch_array($my_likes, MYSQL_ASSOC)){
$job_id = mysql_real_escape_string($myrow['job_id']);
$sql = "SELECT * FROM jobs,sitesettings WHERE  id='$job_id' AND `approved` = 'Yes' ORDER BY `featured` = 'yes' desc ";
if($job_cost){
$sql = "SELECT * FROM jobs,sitesettings WHERE  id='$job_id' AND `approved` = 'Yes' AND `job_cost` = '$job_cost' ORDER BY `featured` = 'yes' desc ";
}
...
echo"<META HTTP-EQUIV = 'Refresh' Content = '0; URL =bookmarks.php'>";
ob_flush();
}
?>
