...
{
$title = "".$row["domain"]." - Tags: ".stripslashes($keyword)."";
$rpp = 15; // results per page
$adjacents = 4;
$page = intval($_GET["page"]);
if(!$page) $page = 1;
$reload = "".$row["site_url"]."/tags?keyword=".$keyword."";
}
$page_name = 'tags?keyword='.$keyword.'';
include("header.php");
include("filter.php");
$job_cost = mysql_real_escape_string($_POST['job_cost']);
$sql = "SELECT * FROM jobs where `approved` = 'Yes'  AND category LIKE '%$keyword%' AND `approved` = 'Yes' OR willdo LIKE '%$keyword%' AND `approved` = 'Yes' OR job_description LIKE '%$keyword%' AND `approved` = 'Yes' OR keywords LIKE '%$keyword%' AND `approved` = 'Yes'";
if($job_cost){
$sql = "SELECT * FROM jobs,sitesettings WHERE `approved` = 'Yes' AND `job_cost` = '$job_cost' AND category LIKE '%$keyword%' AND `approved` = 'Yes' OR username LIKE '%$keyword%' AND `approved` = 'Yes' OR job_description LIKE '%$keyword%' AND `approved` = 'Yes' OR willdo LIKE '%$keyword%' AND `approved` = 'Yes' ";
}
