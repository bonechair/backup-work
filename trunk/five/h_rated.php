...
$page_name = 'h_rated.php';
include("connect.php");
$result = mysql_query("SELECT * FROM sitesettings");
while($row = mysql_fetch_array($result))
{
$title = "".$row["domain"]." - Highest rated ";
...
include("header.php");
include("filter.php");
echo "<h3>".$lang['HIGHEST_RATED']."</h3>";
$job_cost = mysql_real_escape_string($_POST['job_cost']);
$sql = "SELECT * FROM jobs,sitesettings  WHERE `approved` = 'Yes' ORDER BY pos_feed DESC ";
if($job_cost){
$sql = "SELECT * FROM jobs,sitesettings WHERE `approved` = 'Yes' AND `job_cost` = '$job_cost' ORDER BY pos_feed desc ";
}
