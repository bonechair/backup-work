...
{
$title = "".$row["domain"]." - ".$_SESSION['userName']."s completed orders";
...
<div class="orders">
<div class="feedback">
<h2><? echo $_SESSION['userName'];?>'s <?PHP echo $lang['SALES_COMP']?></h2>
<?PHP
$query = "SELECT COUNT(*) as num FROM jobs_sold WHERE `username` = '".mysql_real_escape_string($username)."' AND  `accepted` = 'yes' AND  `feedback_left` = 'yes'";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];
	/* Setup vars for query. */
	$targetpage = "sales_completed"; 	//your file name  (the name of this file)
...
$sql = "select * from jobs_sold WHERE `username` = '".mysql_real_escape_string($username)."' AND  `accepted` = 'yes' AND  `feedback_left` = 'yes' order by id desc LIMIT $start, $limit";
...
<td class=\"td-6\">$datas[username]</td>
<td class=\"td-6\">$datas[accepted]</td>
<td class=\"td-1\">$datas[feedback_left]</td>
...
<img src=\"images/information.png\" class=\"dialog-ico\" alt=\"\"/>".$lang['NO_COMP_SALES']."</div>
</div><br />";
}
 ?>
</div>
<div class="feedback "align="right"><div class="pag"><?=$pagination?></div><input type="submit" class="Button" name="delete" value="Delete Selected"></form>
</div>
<?PHP
if($_POST['delete']=="Delete Selected"){
$query = "DELETE FROM jobs_sold WHERE id IN ('" . implode("','", $_POST['post']) . "')";
...
</div></div><META HTTP-EQUIV = 'Refresh' Content = '0; URL =sales_completed.php'>";
}
}
?>
</div>
